...
dms_to_dd <- function(x) {
  # if we passed NA, its NA
  if (is.na(x) || x == "") return(NA)

  # Clean the string: replace common symbols with a space - no more N S E W etc.
  x_clean <- stringr::str_replace_all(x, "[NSEWnsew°'\"]", " ")

  # Extract all numbers
  nums <- stringr::str_extract_all(x_clean, "\\d+\\.*\\d*")[[1]] |>
    as.numeric()
  ...
  if (stringr::str_detect(x, "[swSW]")) dd <- -dd

  return(dd)
}

#' Updated detect_type to better identify space-separated DMS.
#'@param x Idenitfy the string as
detect_type <- function(x) {
  ...
  if (stringr::str_detect(x_str, "°|'|\"")) return("dms")
  if (stringr::str_detect(x_str, "[NSEWnsew]")) return("dms")

  # Check for space-separated numbers (e.g., "49 17 848")
  # Requires at least one space and the string must start with a digit
  if (stringr::str_detect(x_str, "^\\d+.*\\s+\\d+")) return("dms")
...
convert_coords <- function(df) {
  ...
  df %>%
    mutate(latitude = lat_dd, longitude = -abs(lon_dd)) %>%
